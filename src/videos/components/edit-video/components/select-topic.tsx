...
// import { addTopic, deleteTopic, getTopics } from "src/videos/api";
import { addTopic, getTopics } from "src/videos/api";
import { QUERY_KEY } from "src/common/constants/querykey.constants";

export const SelectTopic = ({ videoId, topics, refetch }) => {
  const [values, setValues] = useState<string[]>([]);
  const [chosenTopic, setChosenTopic] = useState<string>("");
  useEffect(() => {
    setValues(topics.map((topic) => topic.topicKey));
  }, [topics]);
  ...
  const { mutate: mutateAddTopic } = useMutation(addTopic);
  // const { mutate: mutateDeleteTopic } = useMutation(deleteTopic);
  useEffect(() => {
    if (chosenTopic) {
      if (values.includes(chosenTopic)) {
        mutateAddTopic({ topicKey: chosenTopic, videoId });
        refetch();
      } else {
        const idToDelete = {
          videoToTopicId: topics.find((topic) => topic.topicKey === chosenTopic).id,
        };
        // mutateDeleteTopic(idToDelete);
        console.log(idToDelete, "delete");
      }
    }
  }, [chosenTopic]);
  ...
  return (
    <Menu closeOnSelect={false}>
              ...
              {topicsData.map((option) => {
                return (
                  <MenuItemOption
                    ...
                    onClick={() => setChosenTopic(option.description)}
                  >
                    {option.description}
                  </MenuItemOption>
                );
      ...
      <Text>Only add, delete still error, so dont add too much</Text>
    </Menu>
  );
};
